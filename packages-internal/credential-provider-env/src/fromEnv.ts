...
import type { AttributedAwsCredentialIdentity, CredentialProviderOptions } from "@ibm-cos/types";
import { CredentialsProviderError } from "@smithy/property-provider";
import { AwsCredentialIdentityProvider } from "@smithy/types";
import { chain } from "@smithy/property-provider";
import { ibmFromEnv as fromIbmEnv } from "@ibm-cos/credential-provider-env-ibm";
...
const fromAwsEnv =
  ...
  async () => {
    init?.logger?.debug("@ibm-cos/credential-provider-env - fromEnv");
...
export const fromEnv = (init?: FromEnvInit): AwsCredentialIdentityProvider =>
  chain(fromIbmEnv(init), fromAwsEnv(init));
