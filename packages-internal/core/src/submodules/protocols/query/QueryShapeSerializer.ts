...
export class QueryShapeSerializer extends SerdeContextConfig implements ShapeSerializer<string | Uint8Array> {
  ...
  public write(schema: Schema, value: unknown, prefix = ""): void {
    ...
    if (ns.isBlobSchema()) {
      ...
      throw new Error(`@ibm-cos/core/protocols - QuerySerializer unrecognized schema type ${ns.getName(true)}`);
    }
  }

  public flush(): string | Uint8Array {
    if (this.buffer === undefined) {
      throw new Error("@ibm-cos/core/protocols - QuerySerializer cannot flush with nothing written to buffer.");
    }
