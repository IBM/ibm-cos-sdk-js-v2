import { XmlNode, XmlText } from "@ibm-cos/xml-builder";
...
export class XmlShapeSerializer extends SerdeContextConfig implements ShapeSerializer<string | Uint8Array> {
  ...
  private writeStruct(ns: NormalizedSchema, value: unknown, parentXmlns: string | undefined): XmlNode {
    ...
    if (!name || !ns.isStructSchema()) {
      throw new Error(
        `@ibm-cos/core/protocols - xml serializer, cannot write struct with empty name or non-struct, schema=${ns.getName(
          true
        )}.`
      );
    }
        ...
        if (value instanceof XmlNode || value instanceof XmlText) {
          ...
          throw new Error(
            `@aws-sdk - $unknown union member in XML requires ` +
              `value of type string, @ibm-cos/xml-builder::XmlNode or XmlText.`
          );
        }
      }
  ...
  private writeList(
    ...
    if (!listMember.isMemberSchema()) {
      throw new Error(
        `@ibm-cos/core/protocols - xml serializer, cannot write non-member list: ${listMember.getName(true)}`
      );
    }
  ...
  private writeMap(
    ...
    if (!mapMember.isMemberSchema()) {
      throw new Error(
        `@ibm-cos/core/protocols - xml serializer, cannot write non-member map: ${mapMember.getName(true)}`
      );
    }
  ...
  private writeSimple(_schema: ISchema, value: unknown): string {
    if (null === value) {
      throw new Error("@ibm-cos/core/protocols - (XML serializer) cannot write null value.");
    }
    ...
    if (value && typeof value === "object") {
      if (ns.isBlobSchema()) {
        ...
        throw new Error(
          "@ibm-cos/core/protocols - xml serializer, cannot call _write() on List/Map schema, call writeList or writeMap() instead."
        );
      } else {
        throw new Error(
          `@ibm-cos/core/protocols - xml serializer, unhandled schema type for object value and schema: ${ns.getName(
            true
          )}`
        );
      }
