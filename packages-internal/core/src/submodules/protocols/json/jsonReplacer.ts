...
export class JsonReplacer {
  ...
  public createReplacer() {
    if (this.stage === 1) {
      throw new Error("@ibm-cos/core/protocols - JsonReplacer already created.");
    }
    if (this.stage === 2) {
      throw new Error("@ibm-cos/core/protocols - JsonReplacer exhausted.");
    }
    this.stage = 1;
  ...
  public replaceInJson(json: string): string {
    if (this.stage === 0) {
      throw new Error("@ibm-cos/core/protocols - JsonReplacer not created yet.");
    }
    if (this.stage === 2) {
      throw new Error("@ibm-cos/core/protocols - JsonReplacer exhausted.");
    }
