...
import type { AwsCredentialIdentity, CredentialProviderOptions } from "@ibm-cos/types";
...
export const resolveCredentialSource = (
  ...
  const sourceProvidersMap = {
    EcsContainer: async (options?: CredentialProviderOptions) => {
      const { fromHttp } = await import("@ibm-cos/credential-provider-http");
      const { fromContainerMetadata } = await import("@smithy/credential-provider-imds");
      logger?.debug("@ibm-cos/credential-provider-ini - credential_source is EcsContainer");
      return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
    },
    Ec2InstanceMetadata: async (options?: CredentialProviderOptions) => {
      logger?.debug("@ibm-cos/credential-provider-ini - credential_source is Ec2InstanceMetadata");
      const { fromInstanceMetadata } = await import("@smithy/credential-provider-imds");
      return async () => fromInstanceMetadata(options)().then(setNamedProvider);
    },
    Environment: async (options?: CredentialProviderOptions) => {
      logger?.debug("@ibm-cos/credential-provider-ini - credential_source is Environment");
      const { fromEnv } = await import("@ibm-cos/credential-provider-env");
