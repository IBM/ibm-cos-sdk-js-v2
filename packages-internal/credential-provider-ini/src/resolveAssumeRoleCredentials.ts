...
export const resolveAssumeRoleCredentials = async (
  ...
  options.logger?.debug("@ibm-cos/credential-provider-ini - resolveAssumeRoleCredentials (STS)");
  const profileData = profiles[profileName];
  const { source_profile, region } = profileData;

  if (!options.roleAssumer) {
    throw new CredentialsProviderError(
      `Profile ${profileName} requires assuming a role, but no roleAssumer was configured. ` +
      `Provide a roleAssumer function in the credential provider options.`,
  ...
  options.logger?.debug(
    `@ibm-cos/credential-provider-ini - finding credential resolver using ${source_profile ? `source_profile=[${source_profile}]` : `profile=[${profileName}]`
    }`
  );
