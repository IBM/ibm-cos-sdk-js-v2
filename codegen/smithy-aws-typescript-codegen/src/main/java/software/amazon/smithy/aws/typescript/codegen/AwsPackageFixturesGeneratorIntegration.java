...
public final class AwsPackageFixturesGeneratorIntegration implements TypeScriptIntegration {
    ...
    private void writeAdditionalFiles(
        ...
        writerFactory.accept("README.md", writer -> {
            ...
            String operationName = sampleOperation == null
                ? "Example"
    ...
    /**
     * Tries to find an operation with a read-like name prefix "get", "describe",
     * "list" etc.
    ...
    private void writeOperationList(TypeScriptWriter writer, Model model, TypeScriptSettings settings) {
        ...
        for (OperationShape operationShape : operationShapes) {
            ...
            String packageNamePrefix = apiReferencePrefix + "/Package/-" + settings.getPackageName().replace("/", "-");
            if (settings.getPackageName().equals("ibm-cos-sdk-v2")) {
                packageNamePrefix = apiReferencePrefix + "/Package/-aws-sdk-client-" + serviceId;
            }
