...
const topLevelFolders = [packages, packagesInternal];
    ...
    for await (const file of walk(srcPath, ["node_modules"])) {
      ...
      for (const dependency of importedDependencies) {
        ...
        if (
          ...
          if (
            ["@ibm-cos/client-sts", "@ibm-cos/client-sso-oidc", "@ibm-cos/client-signin"].includes(dependency) &&
            ["@ibm-cos/nested-clients"].includes(pkgJson.name)
          ) {
            continue;
          }
