...
for (const submodulePackage of submodulePackages) {
  ...
  for (const submodule of submodules) {
    ...
    if (fs.existsSync(submodulePath) && fs.lstatSync(submodulePath).isDirectory()) {
      ...
      for (const [kind, tsconfig] of Object.entries(tsconfigs)) {
        if (!tsconfig.compilerOptions?.paths?.[`@ibm-cos/${submodulePackage}/${submodule}`]) {
          errors.push(`${submodule} submodule is missing paths entry in tsconfig.${kind}.json`);
          tsconfig.compilerOptions = tsconfig.compilerOptions ?? {};
          tsconfig.compilerOptions.paths = tsconfig.compilerOptions.paths ?? {};

          tsconfig.compilerOptions.paths[`@ibm-cos/${submodulePackage}/${submodule}`] = [
   ...
   * This is a compatibility redirect for contexts that do not understand package.json exports field.
   */
  declare module "@ibm-cos/${submodulePackage}/${submodule}" {
    export * from "@ibm-cos/${submodulePackage}/dist-types/submodules/${submodule}/index.d";
  }
  `
        );
      }
