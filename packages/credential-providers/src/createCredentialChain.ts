 ...
 * @example
 * ```js
 * import { fromEnv, fromIni, createCredentialChain } from '@ibm-cos/credential-providers';
  ...
  const withOptions = Object.assign(baseFunction, {
    expireAfter(milliseconds: number) {
      if (milliseconds < 5 * 60_000) {
        throw new Error(
          "@ibm-cos/credential-providers - createCredentialChain(...).expireAfter(ms) may not be called with a duration lower than five minutes."
        );
      }
