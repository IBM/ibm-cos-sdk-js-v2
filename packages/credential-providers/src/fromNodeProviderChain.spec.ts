...
import { getDefaultRoleAssumer, getDefaultRoleAssumerWithWebIdentity } from "@ibm-cos/nested-clients/sts";
...
vi.mock("@ibm-cos/nested-clients/sts", () => ({
...
vi.mock("@ibm-cos/credential-provider-node", () => ({
...
describe(fromNodeProviderChain.name, () => {
  ...
  it("should defer to @ibm-cos/credential-provider-node", () => {
    const profile = "profile";
    const clientConfig = {
      region: "US_BAR_1",
    };
