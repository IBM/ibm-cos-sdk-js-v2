 ...
 * @internal
 */
export const defaultS3HttpAuthSchemeParametersProvider = async (
  config: S3ClientResolvedConfig,
  context: HandlerExecutionContext,
  input: object
): Promise<S3HttpAuthSchemeParameters> => {
  return {
...
function createIbmAuthIamHttpAuthOption(authParameters: S3HttpAuthSchemeParameters): HttpAuthOption {
  return {
...
export const defaultS3HttpAuthSchemeProvider: S3HttpAuthSchemeProvider = (authParameters) => {
  ...
  switch (authParameters.operation) {
    ...
    default: {
      options.push(createIbmAuthIamHttpAuthOption(authParameters));
      options.push(createAwsAuthSigv4HttpAuthOption(authParameters));
    }
  }
  return options;
